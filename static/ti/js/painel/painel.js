...
function limparTodosFiltros() {
    ...
    const campos = [
        'filtroSolicitante', 'filtroProblema', 'filtroPrioridade',
        'filtroAgenteResponsavel', 'filtroUnidade', 'filtroDataInicio', 'filtroDataFim'
    ];
    ...
    // Re-renderizar chamados
    renderChamadosPage(currentPage);

    // Ativar "Todos os chamados" no menu
    document.querySelectorAll('#submenu-gerenciar-chamados a').forEach(link => {
        link.classList.remove('active');
        if (link.getAttribute('data-status') === 'all') {
...
function initializeStatusFiltering() {
    ...
    document.querySelectorAll('#submenu-gerenciar-chamados a').forEach(link => {
        ...
        newLink.addEventListener('click', function(e) {
            ...
            console.log('Status filtro selecionado:', status);

            currentFilter = status;
            currentPage = 1;

            // Renderizar nova página com filtro
            renderChamadosPage(currentPage);

            // Ativar seção se necessário
            ...
            console.log('Filtro aplicado:', status, 'Total de chamados filtrados:', filterChamados(status).length);
        });
    });
}
...
function validarDadosUsuario(dados) {
    ...
    // Validação melhorada para setores
    if (!dados.setores || !Array.isArray(dados.setores) || dados.setores.length === 0) {
        erros.push('É obrigatório selecionar pelo menos um setor para o usuário');
    }
...
document.getElementById('formCriarUsuario')?.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        console.log('Dados do usuário coletados:', usuarioData);
        ...
        // Validação extra para setores
        if (!usuarioData.setores || usuarioData.setores.length === 0) {
            console.error('Erro: Nenhum setor selecionado');
            throw new Error('É obrigatório selecionar pelo menos um setor para o usuário');
        }
...
['nomeUsuario', 'sobrenomeUsuario', 'nivelAcesso'].forEach(id => {
...
// Validação especial para o campo de setores (múltipla seleção)
const setorUsuario = document.getElementById('setorUsuario');
if (setorUsuario) {
    setorUsuario.addEventListener('change', function() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (btnToggleFiltros && filtrosCard) {
        const cardBody = filtrosCard.querySelector('.card-body');
        ...
        btnToggleFiltros.addEventListener('click', function() {
            if (cardBody.style.display === 'none') {
                ...
                btnToggleFiltros.title = 'Ocultar filtros';
            } else {
                // Ocultar filtros
                cardBody.style.display = 'none';
                icon.className = 'fas fa-eye';
                btnToggleFiltros.title = 'Mostrar filtros';
            }
        });
    }
